...
def get_totp_code():
    ...
    password = totp_tool.generate_totp_code(datetime.now().timestamp() - app.config['base_timestamp'])
    generate_time = totp_tool.get_local_time()
    remain_time = totp_tool.get_remain_time()
    response_content = {"password": password, "generate_time": generate_time, "remain_time": remain_time}
