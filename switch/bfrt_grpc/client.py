...
class KeyTuple:
    """@brief Class to create a KeyTuple. At a time only one of the following cobinations can be set ->
        (value), (value, mask), (value, prefix_len) (low, high). Where each combination corresponds
        to Exact, Ternary, LPM and Range match_type resectively.
    ...
    def __init__(self, name, value=None, mask=None,
            prefix_len=None, low=None, high=None):
...
class _GetParser:
    ...
    def _parse_key(self, key):
        ...
        for key_field in key.fields:
            value = mask = prefix_len = low = high = None
            ...
            key_tuple_list.append(KeyTuple(name, value, mask, prefix_len, low, high))
...
class _Table:
    ...
    def attribute_dyn_key_mask_set(self, target, key, p4_name=None, metadata=None):
        ...
        for field in list(key.field_dict.values()):
            ...
            key_field.mask = bytes(field.value);
